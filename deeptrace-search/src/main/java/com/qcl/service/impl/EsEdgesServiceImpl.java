...
public class EsEdgesServiceImpl implements EsEdgeService {
    ...
    private Query buildQuery(QueryEdgeParam queryEdgeParam) {
        // 构建过滤条件
        List<Query> filterConditions = new ArrayList<>();
        //源节点
        if (queryEdgeParam.getSrcNodeId()!= null){
            filterConditions.add(Query.of(q -> q
                    .match(m -> m
                            .field("src_nodeid")
                            .query(queryEdgeParam.getSrcNodeId())
                    )
            ));
        }
        //目标节点
        if (queryEdgeParam.getDstNodeId()!= null){
            filterConditions.add(Query.of(q -> q
                    .match(m -> m
                            .field("dst_nodeid")
                            .query(queryEdgeParam.getDstNodeId())
                    )
            ));
        }

        // protocols 应用协议
        if (queryEdgeParam.getProtocols() != null && !queryEdgeParam.getProtocols().isEmpty()) {
            filterConditions.add(Query.of(q -> q
                    .terms(t -> t
                            .field("tag.ebpf_tag.protocol.keyword")
                            .terms(t2 -> t2.value(
                                    queryEdgeParam.getProtocols().stream()
                                            .map(FieldValue::of)
        ...
        // endpoints 应用端点
        if (queryEdgeParam.getEndpoints() != null && !queryEdgeParam.getEndpoints().isEmpty()) {
            filterConditions.add(Query.of(q -> q
                    .terms(t -> t
                            .field("tag.ebpf_tag.endpoint.keyword")
                            .terms(t2 -> t2.value(
                                    queryEdgeParam.getEndpoints().stream()
                                            .map(FieldValue::of)
                                            .collect(Collectors.toList())
                            ))
                    )
        ...
        // status codes 响应状态
        if (queryEdgeParam.getStatusCodes() != null && !queryEdgeParam.getStatusCodes().isEmpty()) {
            filterConditions.add(Query.of(q -> q
                    .terms(t -> t
                            .field("status_code.keyword")
                            .terms(t2 -> t2.value(
                                    queryEdgeParam.getStatusCodes().stream()
                                            .map(FieldValue::of)
                                            .collect(Collectors.toList())
                            ))
                    )
            ...
            filterConditions.add(Query.of(q -> q
                    .range(r -> r
                            .term(t-> t
                                    ...
                                    .gte(queryEdgeParam.getStartTime().toString())
                            )
                    )
            ));
        }
        //end_time 结束时间
        if (queryEdgeParam.getEndTime() != null) {
            filterConditions.add(Query.of(q -> q
                    .range(r -> r
                            .term(t-> t
                                    .field("metric.start_time")
                                    .lte(queryEdgeParam.getEndTime().toString())
                            )
                    )
            ));
        }

        // e2e_duration  响应时延  支持范围查询
        if (queryEdgeParam.getMinE2eDuration() != null && queryEdgeParam.getMaxE2eDuration() != null) {
            filterConditions.add(Query.of(q -> q
                    .range(r -> r
                            .term(t-> t
                                    .field("metric.duration")
                                    .gte(queryEdgeParam.getMinE2eDuration().toString())
                                    .lte(queryEdgeParam.getMaxE2eDuration().toString())
                            )
                    )
            ));
        } else if (queryEdgeParam.getMinE2eDuration() != null) {
            filterConditions.add(Query.of(q -> q
                    .range(r -> r
                            .term(t-> t
                                    .field("metric.duration")
        ...
        } else if (queryEdgeParam.getMaxE2eDuration() != null) {
            filterConditions.add(Query.of(q -> q
                    .range(r -> r
                            .term(t-> t
                                    .field("metric.duration")
    ...
    public PageResult<Edges> queryByPage(QueryEdgeParam queryEdgeParam) {
        ...
        if (queryEdgeParam.getEndTime() == null ||
                queryEdgeParam.getEndTime() <= queryEdgeParam.getStartTime()){
            throw new BizException("endTime is required or endTime must be greater than startTime");
        }
        if (queryEdgeParam.getSrcNodeId() == null){
            throw new BizException("srcNodeId is required");
        }
        if (queryEdgeParam.getDstNodeId() ==  null){
            throw new BizException("dstNodeId is required");
        }
        //仅支持按照开始时间和响应时延进行排序
        if (!StringUtils.isEmpty(queryEdgeParam.getSortBy())){
            if (!StringUtils.equalsIgnoreCase(queryEdgeParam.getSortBy(),"metric.duration") || !StringUtils.equalsIgnoreCase(queryEdgeParam.getSortBy(),"metric.start_time")){
                throw new BizException("sortBy is invalid");
            }
        }

        try {
            Query finalQuery = buildQuery(queryEdgeParam);

            // 4.构建动态排序条件
            List<SortOptions> sortOptions = new ArrayList<>();

            if (queryEdgeParam.getSortBy() != null && !queryEdgeParam.getSortBy().isEmpty()) {
                SortOrder order =
                        "desc".equalsIgnoreCase(queryEdgeParam.getSortOrder()) ?
                                SortOrder.Desc :
                                SortOrder.Asc;

                sortOptions.add(SortOptions.of(so -> so
                        .field(f -> f
                                .field(queryEdgeParam.getSortBy())
                                .order(order)
                        )
                ));
            } else {
            ...
            // 6. 提取结果
            List<Edges> edges =  response.hits().hits().stream()
                    .map(Hit::source)
                    .collect(Collectors.toList());
